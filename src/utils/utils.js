"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getProductosConDescuento = void 0;
exports.validateRequiredStrings = validateRequiredStrings;
var sequelize_1 = require("sequelize");
var db_1 = require("../db");
function validateRequiredStrings(obj, fields) {
    console.log("fields es", fields);
    for (var _i = 0, fields_1 = fields; _i < fields_1.length; _i++) {
        var field = fields_1[_i];
        if (obj[field] === null || obj[field] === undefined) {
            return false;
        }
        if (typeof obj[field] === "string" && obj[field].trim() === "") {
            return false;
        }
        if (typeof obj[field] === "number" && isNaN(obj[field])) {
            return false;
        }
    }
    return true;
}
var getProductosConDescuento = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, productosDescuento5Dias, productosDescuento10Dias, productosDescuento15Dias, productoAgrupados, error_1;
    var _b, _c, _d;
    return __generator(this, function (_e) {
        switch (_e.label) {
            case 0:
                _e.trys.push([0, 2, , 3]);
                return [4 /*yield*/, Promise.all([
                        db_1.Producto.findAll({ where: { descuento: (_b = {}, _b[sequelize_1.Op.eq] = 10, _b) } }),
                        db_1.Producto.findAll({ where: { descuento: (_c = {}, _c[sequelize_1.Op.eq] = 20, _c) } }),
                        db_1.Producto.findAll({ where: { descuento: (_d = {}, _d[sequelize_1.Op.eq] = 30, _d) } }),
                    ])];
            case 1:
                _a = _e.sent(), productosDescuento5Dias = _a[0], productosDescuento10Dias = _a[1], productosDescuento15Dias = _a[2];
                productoAgrupados = [
                    { cantidad: productosDescuento5Dias.length, productos: productosDescuento5Dias },
                    { cantidad: productosDescuento10Dias.length, productos: productosDescuento10Dias },
                    { cantidad: productosDescuento15Dias.length, productos: productosDescuento15Dias },
                ];
                res.status(200).json(productoAgrupados);
                return [3 /*break*/, 3];
            case 2:
                error_1 = _e.sent();
                console.log("Error al obtener producsto con descuento ", error_1);
                res.status(500).json({ message: "Erro interno en el servidor" });
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); };
exports.getProductosConDescuento = getProductosConDescuento;
